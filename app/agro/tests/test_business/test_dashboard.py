...
def test_get_dashboard_data(create_farms, create_crop_types):
    ...
    assert (
        dashboard_data["soil_usage"]["total_arable_area_hectares"]
        == expected_arable_area
    )
    assert (
        dashboard_data["soil_usage"]["total_vegetation_area_hectares"]
        == expected_vegetation_area
    )
...
@pytest.mark.django_db
def test_get_dashboard_data_with_farm_having_no_arable_or_vegetation_area(
    create_farmers,
):
...
@pytest.mark.django_db
def test_get_dashboard_data_with_farm_where_arable_and_vegetation_equal_total(
    create_farmers,
):
    farmer = create_farmers[0]
    Farm.objects.create(
...
def test_get_dashboard_data_with_farms_in_all_states(create_farmers):
    ...
    for state in STATE_CHOICES:
        assert any(
            farm["state"] == state[0] for farm in dashboard_data["count_by_state"]
        )
