...
def test_dashboard_api_view(client, create_farms, create_crop_types):
    ...
    assert response.data['total_area_hectares'] == farm1.total_area_hectares + farm2.total_area_hectares
    assert any(item['state'] == farm1.state and item['total'] == 1 for item in response.data['count_by_state'])
    assert any(item['state'] == farm2.state and item['total'] == 1 for item in response.data['count_by_state'])
    assert any(item['crop_type_name'] == crop_type1.name and item['total'] == 1 for item in response.data['farm_count_by_crop'])
    assert any(item['crop_type_name'] == crop_type2.name and item['total'] == 1 for item in response.data['farm_count_by_crop'])
    assert response.data['soil_usage']['total_arable_area_hectares'] == farm1.arable_area_hectares + farm2.arable_area_hectares
    assert response.data['soil_usage']['total_vegetation_area_hectares'] == farm1.vegetation_area_hectares + farm2.vegetation_area_hectares
